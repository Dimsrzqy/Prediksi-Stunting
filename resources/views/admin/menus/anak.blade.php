...
@section('title', __('Data Anak') . ' - Prediksi Stunting')
...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50/50 dark:bg-slate-950/50 p-6 md:p-8 transition-colors duration-300">
    <div class="mx-auto max-w-7xl">
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">{{ __('Manajemen Data Anak') }}</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Kelola daftar anak untuk pencatatan prediksi stunting dan pemantauan.') }}</p>
            </div>

            <div class="flex flex-col sm:flex-row items-center gap-3">
                <a href="{{ route('anak.export') }}" class="flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300 shadow-sm transition-all hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-indigo-200 dark:hover:border-indigo-900 hover:text-indigo-600 dark:hover:text-indigo-400">
        ...
        <div class="flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden transition-colors">
            <div class="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                <h2 class="text-lg font-bold text-slate-800 dark:text-slate-200 tracking-tight">{{ __('Daftar Anak') }}</h2>
            ...
            <div class="overflow-x-auto relative no-scrollbar">
                ...
                <table class="w-full text-left text-sm text-slate-600 dark:text-slate-400">
                    <thead class="bg-slate-50 dark:bg-slate-900/80 text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-500 border-b border-slate-200 dark:border-slate-800">
                        <tr>
                            <th scope="col" class="px-6 py-4">{{ __('NIK') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Nama Anak') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Tgl Lahir / Usia') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Jenis Kelamin') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Orang Tua') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Data Lahir') }}</th>
                            <th scope="col" class="px-6 py-4">{{ __('Pemeriksaan Terakhir') }}</th>
            ...
            <div id="emptyState" class="hidden py-20 px-6 text-center transition-all">
                <div class="flex flex-col items-center justify-center">
                    ...
                    <p class="font-bold text-xl text-slate-800 dark:text-slate-200 tracking-tight">{{ __('Belum Ada Data Anak') }}</p>
                    <p class="text-sm text-slate-500 dark:text-slate-500 mt-2 max-w-xs mx-auto">{{ __('Silakan tambahkan data anak untuk mulai melakukan pemantauan stunting.') }}</p>
                </div>
            </div>
            ...
            <div class="relative transform overflow-hidden rounded-[32px] bg-white dark:bg-slate-900 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-xl border border-slate-200 dark:border-slate-800">
                ...
                <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
                    <div class="flex items-center gap-4">
                        ...
                        <h3 id="modalTitle" class="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">{{ __('Tambah Data Anak') }}</h3>
                    </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                <label for="id_ibu" class="flex justify-between items-center text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">
                                    <span>{{ __('Orang Tua / Ibu Kandung') }} <span class="text-rose-500">*</span></span>
                                    <a href="{{ route('ibu.index') }}" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('Kelola Ibu') }}</a>
                                </label>
                                <select id="id_ibu" name="id_ibu" required class="block w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 border transition-all appearance-none">
                                    <option value="" disabled selected>{{ __('Memuat data orang tua...') }}</option>
                                </select>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    <label for="nik" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">{{ __('NIK') }} <span class="text-rose-500">*</span></label>
                                    <input type="text" id="nik" name="nik" required class="block w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 border transition-all" placeholder="{{ __('16 digit NIK') }}">
                                </div>
                                <div>
                                    <label for="nama_anak" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">{{ __('Nama Lengkap') }} <span class="text-rose-500">*</span></label>
                                    <input type="text" id="nama_anak" name="nama_anak" required class="block w-full rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 border transition-all" placeholder="{{ __('Nama Anak') }}">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                    <label for="tgl_lahir" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">{{ __('Tanggal Lahir') }} <span class="text-rose-500">*</span></label>
                                ...
                                <div>
                                    <label for="jenis_kelamin" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">{{ __('Jenis Kelamin') }} <span class="text-rose-500">*</span></label>
                            ...
                            <div class="pt-4 border-t border-slate-100 dark:border-slate-800">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        <label for="tgl_pemeriksaan" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1">{{ __('Tanggal') }}</label>
...
<script>
    ...
    function renderTable(dataArray) {
        ...
        dataArray.forEach(anak => {
            ...
            if (anak.tgl_lahir) {
                ...
                umurText = months > 0 ? `${months} {{ __('Bulan') }}` : '{{ __('Baru Lahir') }}';
            }

            const genderBadge = (anak.jenis_kelamin === 'L' || anak.jenis_kelamin === 'Laki-laki') ?
                `<span class="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1 text-[11px] font-black text-blue-700 dark:text-blue-400 ring-1 ring-blue-600/20"><i class="fa-solid fa-mars"></i> {{ __('LAKI-LAKI') }}</span>` :
                `<span class="inline-flex items-center gap-1.5 rounded-lg bg-pink-50 dark:bg-pink-900/30 px-2.5 py-1 text-[11px] font-black text-pink-700 dark:text-pink-400 ring-1 ring-pink-600/20"><i class="fa-solid fa-venus"></i> {{ __('PEREMPUAN') }}</span>`;
    ...
    async function deleteData(id) {
        ...
        Swal.fire({
            title: '{{ __('Hapus Data?') }}',
            text: '{{ __('Data anak ini akan dihapus secara permanen.') }}',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        showNotification('{{ __('Gagal menghapus data') }}', true);
                    }
                } catch (error) {
