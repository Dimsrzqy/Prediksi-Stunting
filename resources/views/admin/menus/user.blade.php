...
<main class="flex-1 bg-slate-50/50 dark:bg-slate-950/50 p-6 md:p-8 overflow-y-auto overflow-x-hidden transition-colors duration-300">
    <div class="mx-auto max-w-7xl">
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Manajemen User</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Pusat kendali hak akses untuk menambah, melihat, atau mencabut akses akun sistem.</p>
            </div>
        ...
        <div class="flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
            <div class="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                <h2 class="text-lg font-bold text-slate-800 dark:text-slate-200">Daftar Akun Server</h2>
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50 dark:bg-slate-900/80">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider">Nama</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider">Email (Login)</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider">Nomor HP</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider">Peran (Role)</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider text-right">Aksi</th>
                        </tr>
                    </thead>
        ...
        <div class="flex min-h-full flex-col justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-3xl bg-white dark:bg-slate-900 text-left shadow-2xl transition-all w-full max-w-lg mb-8 sm:mb-auto sm:mt-8 border border-slate-200 dark:border-slate-800">
                ...
                <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
                    <h3 class="text-xl font-bold text-slate-800 dark:text-slate-100">Buat Akun Baru</h3>
                ...
                <div class="p-6">
                    ...
                    <form id="userForm">
                        <div class="space-y-5">
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 ml-1">Nama Lengkap <span class="text-rose-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 ml-1">Email <span class="text-rose-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 ml-1">Nomor Telepon / WhatsApp</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 ml-1">Posisi Jabatan (Role) <span class="text-rose-500">*</span></label>
                        ...
                        <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                            <button type="button" onclick="closeModal()" class="rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">Batal</button>
                            <button type="submit" id="btnSave" class="rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 active:scale-95 transition-all">Simpan Akun</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function renderTable(users) {
        ...
        tbody.innerHTML = '';
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-12 text-center text-sm text-slate-500 dark:text-slate-500"><i class="fa-solid fa-user-slash text-3xl mb-3 block opacity-20"></i> Tidak ada pengguna lain yang terdaftar.</td></tr>';
            return;
        }

        users.forEach((user, index) => {
            const roleBadge = user.role === 'admin' 
                ? '<span class="inline-flex items-center rounded-lg bg-rose-50 dark:bg-rose-900/20 px-2.5 py-1 text-xs font-bold text-rose-700 dark:text-rose-400 ring-1 ring-inset ring-rose-600/20">Admin Pusat</span>'
                : '<span class="inline-flex items-center rounded-lg bg-blue-50 dark:bg-blue-900/20 px-2.5 py-1 text-xs font-bold text-blue-700 dark:text-blue-400 ring-1 ring-inset ring-blue-700/10">User / Ibu</span>';
